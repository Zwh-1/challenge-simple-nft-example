...
-   `DEPLOYER_PRIVATE_KEY`: The private key of the account that will deploy the contracts. **Keep this secret!**
    -   You can generate a new account using `yarn generate` (recommended) or set this variable directly.
...
**`packages/hardhat/.env`**:
-   `DEPLOYER_PRIVATE_KEY`: 用于部署合约的账户私钥。**请务必保密！**
    -   您可以使用 `yarn generate` 生成新账户（推荐），或者直接设置此变量。
