...
/**
 * @dev 市场NFT接口（扩展自MarketplaceListing）
 */
...
export default function Marketplace() {
  ...
  const [listings, setListings] = useState<MarketplaceNFT[]>([]); // 市场列表
  const [loading, setLoading] = useState(true); // 加载状态
  const [buyingTokenId, setBuyingTokenId] = useState<string | null>(null); // 正在购买的代币ID
  // 分页状态：每页 5 个
  ...
  const fetchNFTMetadata = async (tokenId: bigint): Promise<NFTMetadata | undefined> => {
    try {
      ...
      console.error("获取NFT元数据时出错:", error);
    }
    return undefined;
  };
  ...
  const handleMakeOffer = async (listing: MarketplaceNFT) => {
    ...
    try {
      ...
      await makeOffer({
        ...
        value: parseEther(priceStr as any),
      });
      alert("报价已提交！");
  ...
  const filteredListings = useMemo(() => {
    ...
    try {
      min = minPrice ? parseEther(minPrice as any) : null;
    } catch {}
    try {
      max = maxPrice ? parseEther(maxPrice as any) : null;
    } catch {}
  ...
  if (!isClient || loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen">
        ...
        <p className="mt-4 text-lg">加载市场...</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold mb-4">NFT市场</h1>
        <p className="text-lg opacity-70">发现并购买独特的NFT</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="form-control">
            <label className="label">
              <span className="label-text">最低价格 (ETH)</span>
            </label>
            <input
          ...
          <div className="form-control">
            <label className="label">
              <span className="label-text">最高价格 (ETH)</span>
            </label>
            <input
              className="input input-bordered input-sm"
              placeholder=""
          ...
          <div className="form-control">
            <label className="label">
              <span className="label-text">卖家地址包含</span>
            </label>
            <input
              className="input input-bordered input-sm"
              placeholder="0x..."
        ...
        {!bulkMode ? (
          ...
          <div className="flex flex-wrap gap-2 items-center w-full">
            <button
              ...
              onClick={() => setSelectedListingIds(pageListings.map(i => i.listingId))}
            >
              全选本页
            </button>
            ...
            {pageListings.map(listing => (
              ...
              >
                ...
                <figure className="relative overflow-hidden">
                    ...
                    <img
                      ...
                      onError={e => {
                        ...
                        if (target.src.includes("gateway.pinata.cloud")) {
                          target.src = target.src.replace("gateway.pinata.cloud", "ipfs.io");
                        } else if (target.src.includes("ipfs.io")) {
                          target.src = target.src.replace("ipfs.io", "cloudflare-ipfs.com");
                        } else {
                          // 所有网关都失败，显示占位符
                  ...
                  <div
                    className="h-64 w-full bg-gradient-to-br from-primary to-secondary flex items-center justify-center"
                ...
                <div className="card-body p-6">
                  ...
                  {listing.metadata?.description && (
                    <p className="text-sm opacity-70 line-clamp-2 mb-4">{listing.metadata.description}</p>
                  )}
                  ...
                  <div className="mb-4 p-3 bg-base-200 rounded-lg">
                    <div className="flex items-center justify-between">
                      ...
                      <Address address={listing.seller as `0x${string}`} size="sm" />
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between p-3 bg-primary/10 rounded-lg">
                      ...
                      <div className="text-right">
                        <div className="text-lg font-bold text-primary">{formatEther(listing.price)} ETH</div>
                      </div>
                    </div>
                      ...
                      <div className="p-3 bg-base-200 rounded-lg space-y-2">
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            onChange={e =>
                              setEditPrices(prev => ({ ...prev, [listing.listingId.toString()]: e.target.value }))
                            }
                          />
                          <button
                      ...
                      <div className="p-3 bg-base-200 rounded-lg space-y-2">
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            onChange={e =>
                              setOfferPrices(prev => ({ ...prev, [listing.listingId.toString()]: e.target.value }))
                            }
                          />
                          <button
      ...
      {(offers as any[]).map((offer, idx) => {
        ...
        return (
          <div key={key} className="flex items-center justify-between text-sm bg-base-300 rounded px-2 py-1">
            <div className="flex items-center gap-2">
              <Address address={offerer as `0x${string}`} format="short" />
