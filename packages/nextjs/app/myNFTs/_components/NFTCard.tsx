      ...
      await writeMarketplaceContract({
        ...
        args: [
          (() => {
            ...
            if (!resolved)
              throw new Error("无法获取NFT合约地址，请检查部署或环境变量 NEXT_PUBLIC_YOUR_COLLECTIBLE_ADDRESS");
            return resolved;
          })(),
          BigInt(nft.id.toString()),
          parseEther(minBid),
      ...
      <figure className="relative overflow-hidden">
        ...
        {nft.attributes && nft.attributes.length > 0 && (
          <div className="absolute top-4 right-4">
            <div className="badge badge-primary badge-sm">{nft.attributes[0]?.value}</div>
          </div>
        )}
        {nft.isListed && (
          <div className="absolute top-4 left-4">
            <div className="badge badge-secondary font-bold">已上架</div>
          </div>
        )}
      </figure>
          ...
          <div className="mb-4">
            <div className="flex flex-wrap gap-2">
              ...
              {nft.attributes.length > 3 && (
                <div className="badge badge-ghost badge-sm">+{nft.attributes.length - 3} more</div>
              )}
            </div>
          </div>
        )}
        ...
        <div className="card-actions flex-col">
          {!showTransfer && !showSell && !showBlind ? (
            <div className="flex gap-2 w-full">
              <button className="btn btn-outline btn-sm flex-1" onClick={() => setShowTransfer(true)}>
              ...
              {nft.isListed ? (
                <button className="btn btn-disabled btn-sm flex-[2] opacity-50 cursor-not-allowed">已上架</button>
              ) : (
                <>
                  ...
                  <button className="btn btn-secondary btn-sm flex-1" onClick={() => setShowBlind(true)}>
                    <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button
                  ...
                  onClick={() => {
                    try {
                      ...
                      console.error("调用transferFrom函数错误", err);
                    }
                  }}
                >
